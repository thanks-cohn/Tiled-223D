...
**Flight controls:** W/S forward/back, A/D turn, Up/Down ascend/descend, Shift boost, R take off, and C swaps the main and live-preview cameras in both directions. Import your Tiled map JSON and its separate elevation JSON through the on-screen buttons. The ship defaults to a low-poly sphere; add `public/ship/ship.glb` to replace it.
