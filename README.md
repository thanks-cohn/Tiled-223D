...
Ocean Flight V1.1 / V10 now uses four deterministic, world-anchored ribbon families and four smoothly overlapping altitude moods, plus a soft shoreline-sampled height footprint. LOW uses restrained connected crests, MIDDLE coordinates three scales, HIGH uses broad blue-on-blue bands, and TOP defaults to sparse tonal contours with negligible white. Ocean Studio and the authorized development API expose validated, reversible width, continuity, color, crest/tonal, mood and shadow controls without changing pilot/world truth. The renderer retains one sea surface and bounded buffers (up to four family draw calls plus one shadow footprint); browser aesthetic acceptance and 4 GB device performance still require owner verification. See the [Ocean Flight V1.1 agent contract](Semantic-Bindings/ocean-flight-v1-agent-contract.md) and [beauty-pass tuning map](docs/OCEAN_STUDIO_TUNING_GUIDE.md).
