...
export function validateRegion(region, project, ignoringId = null) {
  ...
  for (const existing of project.regions) if (existing.id !== ignoringId && boundsOverlap(region.bounds, existing.bounds)) {
    const code = existing.locked ? ERROR_CODES.locked : ERROR_CODES.overlap;
    throw new WorldApiError(code, `${region.id} overlaps ${existing.id}${existing.locked ? ', which is locked' : ''}.`, ['Move the proposed footprint.', 'Explicitly replace an unlocked region by ID.']);
  }
}
