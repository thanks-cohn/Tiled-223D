...
export function islandVisualTransform(ship,worldPosition,curvature,globeReveal){
 const dx=worldPosition.x-ship.x,dz=worldPosition.z-ship.z;
 const distance=Math.hypot(dx,dz);
 const fraction=THREE.MathUtils.clamp(globeReveal,0,1);
 const spherical=globeSurface(distance,GLOBE_RADIUS);
 const low=curvatureDrop(distance,THREE.MathUtils.clamp(curvature,0,1));
 ...
 const arc=Math.min(distance/GLOBE_RADIUS,Math.PI);
...
export function positionIslandVisual(item,group,ship,profile){
 ...
 const transform=islandVisualTransform(ship,base,profile.curvature,profile.globeReveal);
