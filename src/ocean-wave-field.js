...
export function sampleWaveGroup(family,cellX,cellZ,{worldWidth,worldHeight,seed=223,worldScale=1}={}){
 ...
 const bands=[];for(let band=0;band<family.fragments;band++){const offset=(band-(family.fragments-1)/2)*family.groupSpacing,bandScale=1-band*.09;const points=[];for(let i=0;i<=12;i++){const q=i/12*2-1,gap=hash(cellX*17+i,cellZ*13+band,familySeed+5)<family.gapRhythm&&Math.abs(q)>.36;points.push({x:anchor.x+tangent.x*q*length*bandScale/2+normal.x*(offset+length*family.curvature*(1-q*q)),y:.18,z:anchor.z+tangent.z*q*length*bandScale/2+normal.z*(offset+length*family.curvature*(1-q*q)),visible:!gap,taper:Math.pow(Math.max(0,1-Math.abs(q)),family.taper*.8+.2)});}bands.push(points);}
 return {id:waveIdentity(family.id,cellX,cellZ,worldWidth,worldHeight,familySeed),familyId:family.id,mode:family.mode,seed:familySeed,cell:{x:cellX,z:cellZ},anchor,angle,bands,space:"authoritative-ocean-world",sourceControls:{grid,length,curvature:family.curvature,groupSpacing:family.groupSpacing,width:family.width,softness:family.softness,taper:family.taper,gapRhythm:family.gapRhythm}};
}
