...
These values describe **presentation layers, not world terrain elevations or collisions**. Near, middle, and high clouds exist at separate heights and horizontal scales. Near clouds move past a fast ship more rapidly in the image because they are spatially closer; high clouds remain distant. Independent low/mid/high/planet wind advection rates (0.36 / 0.15 / 0.045 / 0.018 units per simulation second) add gentle different drift without manufacturing the ship's movement. The model's `relativeMotion` numbers are semantic diagnostics, not arbitrary speed multipliers applied to the actual ship or collision world.
