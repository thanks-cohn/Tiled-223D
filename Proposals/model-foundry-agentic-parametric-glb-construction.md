...
## Tiled-223D / 223D integration

Keep the Foundry separate from the Tiled terrain/world generator and the browser flight runtime. Provide a standalone reference/prose→editable-spec→GLB path whose semantic export optionally maps engine nozzles, camera anchors, docking/landing locations, bounds, LOD and collision interfaces into the existing ship-loading workflow (e.g. a generated `ship.glb`). Do not presume that the flight engine currently implements the Foundry or that a GLB alone encodes gameplay behavior. Let low-end machines choose Default while authoring detailed or MINUTIA assets for selective display/inspection, not blanket all-world mesh density.
