...
## Tiled-223D implementation entry points (proposal only)

Prototype within the existing V9 development branch, after comparing the user's screenshot and reviewing `src/ocean-crest-field.js`, `src/ocean-speed-cues.js`, `src/horizon.js`, `src/flight-model.js`, `src/atmosphere-model.js`, `src/cinematic-flight-framing.js` and `src/spatial-development.js`. The currently implemented two fixed crest lattices and single curved ocean mesh are **baseline**, not evidence that the four-family/shadow design exists. Extend existing diagnostics and `Bugs/`; keep Current/Bigger camera behavior and authoritative geometry unchanged. Future FrameChute/SUBSTRATE adapters should consume the same declared spatial state rather than imitating a screenshot.
