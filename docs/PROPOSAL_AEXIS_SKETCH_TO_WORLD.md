...
**Tiled-223D integration note:** This describes future features, not current capabilities. The present prototype has a procedurally generated 500 × 500 JS world, Tiled imports, and a Windows start-world script; it does **not** yet implement the visual draft editor, the proposed draft-to-JS generator, or portable chunk placement. Align first with [the Worldweaver proposal](PROPOSAL_AEXIS_WORLDWEAVER.md), [schema](SCHEMA.md), and [agent handoff](CODEX_HANDOFF.md); retain 2D terrain/elevation truth, chunk rendering and offline operation.
