...
## Cinematic Ocean V1 handoff

The former two fixed crest pools have been superseded by four deterministic geographic families, continuous LOW/MIDDLE/HIGH/TOP mood weights, and one soft vertical-under-ship ocean footprint. `src/ocean-visual-controller.js` owns lifecycle and composes pure preset/mood/field/shadow modules with bounded renderers. Read-only state is at `window.tiledSpatial.ocean`; mutation remains behind the existing trusted-click capability as `window.tiledSpatialDevelopment.ocean*`. The Ocean Studio uses that same API. Start future work at `Semantic-Bindings/ocean-flight-v1-agent-contract.md`, and update its schema, tests and truthful handoff with every preset or placement change. Automated tests/build pass, but visual acceptance and target 4 GB Windows profiling remain outstanding.

V10 replaces hair-like one-pixel crests with pooled tapered ribbons: restrained connected crest bands at LOW and broad blue-on-blue tonal families at MIDDLE/HIGH/TOP. TOP's white contribution is negligible by default. The sole preset now exposes ribbon width, taper, deterministic gaps, softness, glint, per-mood white/tonal strength, and shoreline-sampled shadow controls. See `docs/CODEX_OCEAN_BEAUTY_PASS.md` and the tuning guide; browser aesthetic approval and 4 GB Windows profiling remain owner gates.

## Shadow & circular-artifact Deep Debug V10

A capture-on-demand `shadow-deep-debug-v1` collector now extends Spatial Truth. It records the physical→model→mesh/material→shared-horizon→camera/viewport chain and broad-phase base-ocean/ribbon/atmosphere contributors on one completed frame. Read-only inspection is mode restricted; capture, transition reports and reversible shadow-only isolation use the existing trusted-click scopes. Performance does not invoke the collector. No physics, ocean preset, wave, cloud or camera defaults changed. Automated tests/build pass locally; real browser comparison, GPU pixel attribution, Actions, owner visual approval and 4 GB Windows profiling remain outstanding.
