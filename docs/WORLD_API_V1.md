...
All project reads now require an explicit actor with `inspect`: this includes the legacy `AgentWorldApi.inspect(actor)`, `ProgrammerWorldApi.inspectMap(actor, bounds)`, paged queries, placement fixtures, and `explainCell(actor, {x,y})`. Map inspection has an implementation-owned 4,096-cell cap; callers cannot raise it. Pages contain at most 100 records and are sliced before cloning. **Compatibility change:** legacy `inspect().regions` is now a 25-item page object (`items`, `total`, and `nextOffset`) rather than an unbounded array; use `inspectRegions` for subsequent pages.

Requests may select `debug: "off" | "regular" | "deep"` (CLI: `--debug`). Other values are rejected as `INVALID_REQUEST`, never downgraded. This changes only the view of the core decision. A diagnostic-v1 record always carries stable code/severity/message, phase, operation/project/revision context, affected IDs or coordinates, and a repair hint. Deep mode adds the ordered trace actually captured at validation and transaction decision sites, capped at 32 steps; `truncated` is true only after a 33rd step was omitted. Regular reports the same authoritative code without building a trace. Records are capped at 8, strings at 512 characters, repairs at 4, arrays/fact keys at 32/16, and diagnostic nesting at 4. Operation, actor, and region IDs are limited to 128 characters. Debug selection grants no permissions and never changes mutation results, project files, revision, or operation log.
...
Single-cell explanation reads only the requested base cell and regions that write it. Bounded map inspection copies only requested base cells and overlays only regions intersecting the rectangle; it no longer clones/projects the entire map. Placement-surface work remains proportional to the requested region footprint.
...
Schema IDs in capability output use local transport identifiers such as `world-api-types-v1#/$defs/pageRequest`; resolve the name to the corresponding repository schema before applying the JSON Pointer. CLI failures are printed as a bare diagnostic-v1 record to stderr, while library mutation calls either return a transaction result containing `diagnostics` or throw `WorldApiError` with an attached `diagnostic`. That transport envelope difference does not change the diagnostic record schema.

### Repeatable 500 × 500 read benchmark

Run `node --expose-gc scripts/benchmark-world-api.mjs`. It constructs an in-memory 500 × 500 ocean world with 100 deterministic 10 × 10 regions, forces GC before each group when available, and reports wall-clock elapsed/mean time plus the non-GC-normalized `heapUsed` delta. On Linux x64, Node v20.20.2 in this development container, one run measured: 1,000 `explain-cell` calls in 7.309 ms (1,776,000-byte heap delta), 1,000 1 × 1 `inspect-map` calls in 3.920 ms (1,320,408 bytes), and twenty 64 × 64 reads in 11.804 ms (4,404,488 bytes). Results are workload and runtime dependent; heap deltas include allocator/GC noise. This is not Windows or 4 GB hardware validation.
