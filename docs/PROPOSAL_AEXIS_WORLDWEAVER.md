# Tiled-223D proposal: ÆXIS Worldweaver — anchor maps into coherent generated geography

**Status:** Future proposal only; not implemented in the existing browser world.  
**Repository placement:** `docs/PROPOSAL_AEXIS_WORLDWEAVER.md`.  
**Related:** [existing world-scale creator-authored islands proposal](PROPOSAL_DOCKING_AND_CINEMATICS.md#addendum--creator-authored-islands-agent-generated-expansive-geography), [semantic terrain data](SCHEMA.md), and [spatial geometry invariants](SPATIAL_GEOMETRY.md).

**Tiled-223D integration notes:** Use the existing Tiled importer and elevation/semantic IDs as an eventual on-ramp, not a claim that the generator already exists. Add a separate authoring-time constraint and procedural generation layer; keep creator maps immutable by default and world XY tile coordinates/elevation independent of the displayed 2D/3D camera. Respect existing world-data and chunk/rendering budgets, and do not allocate a million individual meshes or an entire multi-continent tile grid. The ocean-between-two-places/same-continent case must honor the creator's selected semantics rather than redefining "continent" silently. Preserve an offline, no-LLM workflow even when agent assistance is available.
