...
import {loadStored,persistProfile} from "./dirt-compiled-storage.mjs";
import {hydrateProduction} from "../src/dirt/compiled.js";
...
const core=new DirtWorldCore(createDirtState(saved));
await loadStored(core,arg("compiled-dir")||"generated/dirt-compiled");
if(!fs.existsSync(statePath)){const production=hydrateProduction(core.compiledSource);core.state.production=production;core.state.rules=production.rules;}
const request=JSON.parse(fs.readFileSync(path.resolve(requestPath),"utf8")),response=core.execute(request);
if(response.status==="ok"&&request.operation==="dirt.compileProfile"){
 response.result.persistence=persistProfile(core,request.input.plan.worldId,arg("compiled-dir")||"generated/dirt-compiled");response.result.persisted=true;
}
