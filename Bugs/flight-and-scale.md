...
## OCEAN-004 — ALT ~374 circular formation / globe skirt (V10 candidate fix)

- **Reported:** On the 500 × 500 world around ALT ~374 and after expansive→Current transitions, the creator sees a giant blue circular formation and several smaller circular overlays. The overall ocean visuals are otherwise pleasing; the stylized under-ship shadow is sometimes too large.
- **Verified source issue:** `buildOceanGeometry(R)` used a minimum outer radius of 925 (and 1.02 × πR at larger scales), while the common globe vertex shader clamps its spherical arc at π. At R=235, the canonical antipode is π×235≈738.27, leaving 186.73 world units of ocean rings **outside the valid globe-projection domain**. At ALT 374 the globe blend is about 0.734: those rings still carry planar displacement and fold back into a visible extra skirt. This is a mathematical geometry-domain defect; it does **not** prove which individual screenshot pixels were shadow.
- **Candidate correction on `fix/ocean-globe-domain-v10`:** End the same single radial ocean mesh at exactly πR on all three scales. Keep sectors/rings fixed, preserve physical ocean/terrain/world data, maintain the full 500-map width within the 738-unit current-scale sea, and do not change shadow presets, clouds, ship, camera, island geometry, or flight physics. Added `tests/ocean-globe-domain.test.js` to prevent any sea rings beyond the antipode.
- **Manual gate before closing:** Test identical ALT ~320/374/420/500 views at Current, Bigger, Massive, plus expansive→Current. In Deep Debug, capture the affected frame, inspect `tiledSpatial.shadow.getContributors()`, and temporarily hide only `ship-ocean-shadow` via the authorized `shadowIsolatePreview`/restore API. If a circle remains when the shadow is hidden, attribute it to other sea/globe/tonal geometry before adjusting shadow radius. Do not claim the original issue resolved solely from these source-level and automated tests. Ask the creator to confirm visuals and 4 GB performance on their device.
